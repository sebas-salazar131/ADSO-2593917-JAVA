<?php 
    ...
    if (!empty($_POST['cedula']) and !empty($_POST['nombres']) and !empty($_POST['apellidos']) and !empty($_POST['telefono']) and !empty($_POST['direccion']) and !empty($_POST['email'])) {
        ...
        $consulta = $base_de_datos->prepare("UPDATE personas SET nombres=:nom, apellidos=:ape, telefono=:tel, direccion=:dir, email=:email WHERE cedula=:doc");
        ...
        if( $proceso ){
            $respuesta['respuesta'] = [
                                        'status' => true,
                                        'mesagge' => "OK##Cliente##UPDATE",
                                      ];
            echo json_encode($respuesta);
        }else{
            $respuesta['respuesta'] = [
                                        'status' => false,
                                        'mesagge' => "ERROR##Cliente##UPDATE",
                                      ];
            echo json_encode($respuesta);
        }
