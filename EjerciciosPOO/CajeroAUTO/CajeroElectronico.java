...
public class CajeroElectronico{
	...
	public void consultarSaldoTarjeta(TarjetaDebito tarje, String contra){
		//validar numero de tarjeta
		   //si
		   if ( tarje.verificarPassword(contra) ) {
			tarje.mostrarSaldo();
			this.registrarLog("Mostrar Saldo", tarje.getNumero(), tarje.getSaldo(), "OK:200");
			//no
		   }else{
			System.out.println(" ===== ERROR - PASSWORD INCORRECTO =====");
			this.registrarLog("MOSTRAR SALDO", tarje.getNumero(), tarje.getSaldo(), "Error:403");
		   }
		   
	}

	public void cambiarClave(TarjetaDebito tarje, String vieja_contra, String nueva_contra ){
		if(tarje.verificarPassword(vieja_contra)){
			tarje.setPassword(nueva_contra);
			System.out.println("El cambio de la contrasena ha sido exitoso");
			this.registrarLog("Cambio de contrasena", tarje.getNumero(), 00000000, "OK:200");
		}else{
			System.out.println(" ===== ERROR - PASSWORD INCORRECTO =====");
			this.registrarLog("Cambio de contrasena", tarje.getNumero(), 00000000, "Error:403");
		}
	}

	public void transferirDineroaTarjeta(TarjetaDebito tarje, int cant_10, int cant_20, int cant_50, int cant_100){
	    int resultado = (10000*cant_10)+(20000*cant_20)+(50000*cant_50)+(100000*cant_100);
		tarje.setSaldo(resultado+tarje.getSaldo());
		this.registrarLog("Saldo aumentado", tarje.getNumero(), resultado, "OK:200");
		this.cant_10+=cant_10;
		this.cant_20+=cant_20;
		this.cant_50+=cant_50;
		this.cant_100+=cant_100;
